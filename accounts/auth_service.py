"""Compatibility wrapper: re-export canonical auth service implemented in `auth`.

This keeps historical imports like `accounts.auth_service` working while centralizing
the implementation in `auth/auth_service.py`.
"""
...
from accounts.api.config import API_AUTH_LOGIN, API_BASE_URL, API_VERIFY_TLS, API_TIMEOUT, API_RETRIES
...
def authenticate(account_type: str, email: str, password: str) -> Tuple[bool, str, Optional[dict]]:
    """Authenticate with the API.

    Returns (success: bool, message: str, response_data: dict or None).
    """
    url = API_AUTH_LOGIN
    ...
    try:
        ...
        response = session.post(url, json=payload, headers=headers, timeout=API_TIMEOUT, verify=API_VERIFY_TLS)

        logger.debug("Status Code: %s", response.status_code)
        
        # Print raw response for debugging (defensive: don't raise on weird test doubles)
        try:
            print("\n" + "="*60)
            print("BACKEND LOGIN RESPONSE")
            print("="*60)
            print(f"Status Code: {getattr(response, 'status_code', None)}")
            # Some test doubles may not expose `headers` or may provide a non-mapping
            try:
            ...
            try:
                # response.json() may raise or be missing on mocks
            ...
            # Swallow debug printing errors so they don't break tests
        ...
        try:
            ...
            return False, "Invalid server response structure.", None

        # Check if login was successful based on statusCode or presence of userId
        if validated.statusCode == 200 or validated.data.userId:
            # Store token if available
            token = validated.data.token or validated.data.accessToken
            if token:
                _store_token(token, email)
            # Return full response data for processing
            return True, validated.message or "Login successful", payload_json
        else:
            return False, validated.message or "Login failed.", None
...
def get_token() -> Optional[str]:
    ...
    # As a last resort, try reading the token from the in-memory AppStore
    try:
        from accounts.store import AppStore
        store = AppStore.instance()
        user = store.state.get("user", {}) if store else {}
        token = user.get("accessToken") or user.get("token")
        if token:
            return token
    except Exception:
        # Ignore errors here; return None at the end
        logger.debug("AppStore token fallback not available", exc_info=False)

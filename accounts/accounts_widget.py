...
from Left_panel.accounts.login_page import LoginPage
from Left_panel.accounts.store import AppStore
from Left_panel.accounts.accounts_tree import build_accounts_tree
from Left_panel.accounts.add_user_dialog import AddUserDialog
from Left_panel.accounts.remove_user_dialog import RemoveUserDialog
from Left_panel.accounts.switch_account_dialog import SwitchAccountDialog
from Left_panel.accounts.login_worker import LoginWorker
...
class AccountsWidget(QWidget):
    ...
    def show_login(self):
        import sys
        print("[DEBUG] *** show_login() CALLED ***", flush=True)
        sys.stdout.flush()
        
        login_dialog = LoginPage(self)
        print(f"[DEBUG] Created LoginPage: {login_dialog}", flush=True)
        sys.stdout.flush()
        
        login_dialog.login_success.connect(self.on_login_success)
        
        # Connect create account signal
        print("[DEBUG] Attempting to connect create_account_requested signal...", flush=True)
        sys.stdout.flush()
        
        try:
            print("[DEBUG] Importing CreateAccountDialog...", flush=True)
            sys.stdout.flush()
            from Create_Account import CreateAccountDialog
            print(f"[DEBUG] CreateAccountDialog imported: {CreateAccountDialog}", flush=True)
            sys.stdout.flush()
            
            def on_create_account():
                print("[DEBUG] *** HANDLER CALLED: on_create_account() ***", flush=True)
                sys.stdout.flush()
                d = CreateAccountDialog(self)
                print(f"[DEBUG] CreateAccountDialog instance created", flush=True)
                sys.stdout.flush()
                d.exec()
                print(f"[DEBUG] CreateAccountDialog.exec() finished", flush=True)
                sys.stdout.flush()
            
            print(f"[DEBUG] Connecting signal...", flush=True)
            sys.stdout.flush()
            login_dialog.create_account_requested.connect(on_create_account)
            print("[DEBUG] *** create_account_requested signal CONNECTED SUCCESS ***", flush=True)
            sys.stdout.flush()
        ...
        try:
            ...
            print("[DEBUG] Connected forgot_password_requested signal", flush=True)
            sys.stdout.flush()
        except Exception as e:
            print(f"[DEBUG] Failed to connect forgot_password: {e}", flush=True)
            sys.stdout.flush()

        print("[DEBUG] Calling login_dialog.exec()...", flush=True)
        sys.stdout.flush()
        result = login_dialog.exec()
        print(f"[DEBUG] login_dialog.exec() returned: {result}", flush=True)
        sys.stdout.flush()
        return result
    ...
    def on_verify_complete(self, success, message, response_data, account_number, email, expected_owner_email, is_trade_password=False):
        ...
        self.store.set_current_account(account_number, email.lower(), is_own=False, account_id=None, is_demo=False)
